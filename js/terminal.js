...
class Terminal {
        ...
        if(val.startsWith("cd ")) { // change directory
            ...
            name.forEach((element)=>{
                if(element.includes(",")||element.includes("]")||element.includes("[")||element.includes("}")||element.includes("{")||element.startsWith("file::")||Object.keys(folders).includes(element)) { // TODO Update for localForage
                    ...
                    // ! This code is taken from the FileViewer class's method called _addFileToDifferentLocation. If that is changed this should to.
                    let currentFolders = localStorage.getItem('folders');
                    localStorage.setItem('folders', currentFolders+" ["+element+"]{}");
                    // Next, add a new subfolder of the current directory.
                    currentFolders = localStorage.getItem('folders');
                    localStorage.setItem('folders', currentFolders.replace(this.currentFolder+"]{", this.currentFolder+"]{"+element+","));
                    // Finally, add the properties to folders{} so they will display
                    folders[element] = [];
                    folders["parent-"+element] = this.currentFolder;
                    folders[this.currentFolder].unshift(element); // ? push? It looks different after the page has been reloaded if push is used
                    // TODO if added to desktop, reload the desktop
                }
            });
    ...
    getFolderFromPath(newPath, showErrors=true) {
        ...
        let lastStableFolder = this.currentFolder;
        if(newPath.startsWith("/")) {
            if(newPath == "/") {
                let out = {};
                out["landingFolder"] = "usr";
                out["path"] = "/usr";
                return out;
                // below code isn't run if this is true because of how return works
            }
            // start at root directory (usr as of now)
            curFolder = "/usr";
            lastStableFolder = "usr";
        } 
        ...
        newPath.forEach((element)=>{ // TODO update to include parent when localForage
            if(element) { // prevents empty strings from being processed
                // travel to folder
                if(curFolder !== false) {
                    if(folders[lastStableFolder].includes(element)) {
                        // element is in current directory
                        curFolder += "/"+element;
                        lastStableFolder = element;
                    } else if(folders["parent-"+this.currentFolder] == element) {
        ...
        if(curFolder !== false) {
            let out = {};
            out["landingFolder"] = curFolder.substring(curFolder.lastIndexOf("/")+1); // Todo update this for backslashes too
            out["path"] = curFolder;
            return out;
        } else {
            return undefined;
